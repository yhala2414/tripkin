...
- Do not let duplicated guidance drift silently. When the same rule appears in multiple docs, identify the source of truth, sync or remove stale copies, and record how future updates should propagate.
...
3. **Audit duplication and drift risk**
   - Identify rules, decisions, routes, workflows, commands, scope gates, and verification requirements that appear in more than one place.
   - For each duplicated rule, decide whether duplication is intentional reinforcement or accidental copy-paste.
   - Assign one source of truth for each rule. Other mentions should be summaries, links, or clearly scoped restatements.
   - If copies disagree, treat it as a collaboration-system defect: sync the stale text, remove unnecessary repetition, or add a propagation note explaining which documents must be updated together.
   - Prefer reducing duplicated binding rules over making every document comprehensive.

4. **Define stage boundaries**
...
7. **Define verification contract**
...
8. **Use methodology skills when available**
...
| Documentation drift failure     | The same rule existed in multiple places and one copy became stale.       | Pick a source of truth, sync or remove stale copies, and add an update path. |
...
- Existing guidance is duplicated across files: choose a source of truth, replace secondary copies with summaries or links, and define a sync rule for intentional repetition.
...
Correction loop:
Duplicated guidance / drift risks:
Known failure types:
...
- Repeated rules to consolidate, link, or intentionally keep in sync.
...
- **Duplicate-rule drift:** A rule appears in several formal docs, but only one copy was updated after a decision changed.
...
| Repeating binding rules everywhere           | Choose a source of truth; keep secondary mentions short, linked, and synced only when intentional. |
...
- Duplicated guidance has been audited, with a source of truth or sync path for intentional repetition.
